module Authors
    class AccountsController < AuthorController
        ...
        def change_password
            if current_author.valid_password?(author_password_params[:current_password])
                current_author.update(
                    ...
                    password_confirmation: author_password_params[:nwe_password_confirmation]
                )
                end
                redirect_to authors_account_path
        ...
        def author_password_params
            params.require(:author).permit(:current_password, :new_password, :new_confirmation_password)
        end    
